...
| :--- | :--- | :--- |
| **Database & API** | Supabase (PostgreSQL) | Relational storage and auto-REST API generation. |
| **Media Storage** | Cloudinary | Image hosting and optimization. |
| **Orchestration** | Apache Airflow | DAG-based scheduling, task dependencies, and resilience. |
| **Scraping** | Puppeteer | Extracting raw menus and images from dynamic websites. |
| **Vision & OCR** | YOLO & Tesseract.js | Local execution for food classification and text extraction. |
| **Containerization** | Docker Compose | Secure, isolated environments for application deployment. |
| **Frontend** | Next.js & Vercel | Public interface and crowdsourcing form hosting. |
...
Never commit your `.env` file to version control. Create a `.env` file in the root directory and populate it with your local database credentials and API keys:
